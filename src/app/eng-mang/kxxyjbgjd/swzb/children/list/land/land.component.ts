...
export class LandComponent implements OnInit {
    ...
    public bj_land_new_data; //初始化新增数据标记
    public flxx_land_update = new Array(); //分类信息
    public flxx_land_add = new Array(); //分类信息新增
    public flxx_land_del = new Array(); //分类信息删除
    ...
    public land_name_active_base_copy;
    public land_name_active_base_copy2;
    public szxzqugldm;
    ...
    //房屋性质
    public TdxzTableList: any;
    public TdxzTreeList: any;
    public isShowTdxz = false;
    //房屋主要用途
...
// 订阅表单值改变事件
            ...
            if(this.landList.length>0){
                ...
                this.ggmxlist.valueChanges.debounceTime(500).subscribe( data => {
                    ...
                    this.ggmxTotals=0;
                    this.ggmxtgmjTotals=0;
                    this.ggmx.forEach((item,key,arr) => {
                        console.log(item.txmj);

                        if(item.txmj){
                            this.ggmxTotals +=item.txmj;
                        }

                        if(item.tgsmj){
                            this.ggmxtgmjTotals +=item.tgsmj;
                        }
                    });
    ...
    ngOnInit() {
        ...
        //
        // let oninit_res = this.TabCommon.Oninit(this.data,this.name_active_data,this.landList,this.name_active_base,this.bj_land_data);
        //
        ...
        // this.landList = oninit_res.List;
        // this.bj_land_data = oninit_res.bj_data;

        if(this.data['returnObject']['datalist']){
            if(this.data['returnObject']['datalist'][0] !=undefined ){


                this.name_active_data = [ new LandData('','','','','','',new Date().getTime(),'','','','',[],'','','','','','','','','','','','','','','','','','','','','','') ];
                ...
                if(this.init_land_data['datalist'] != undefined && this.init_land_data['datalist'].length>0){

                    this.init_name_active_base = this.init_land_data['datalist'][0]['list'];
                }
...
// =============== end 规格明细  =======================
        ...
        if(this.bj_land_data == 1) {
            ...
            if(this.data['returnObject']['datalist'] && this.data['returnObject']['datalist'].length>0){
                ...
                this.landList.forEach((value,index,arr)=>{
                    console.log(value);
            ...
            if(this.init_land_data['datalist'] != undefined && this.init_land_data['datalist'].length >0){
                this.init_land_data['datalist'].forEach((value,index,arr)=>{
                    value.list.forEach((val,i,ar)=>{
                        ar[i] = this.DataProcessing.returnTreeTable(this.DataProcessing.replaceChildlValue(val, 'childList', 'children', '', ''));
            ...
            this.init_land_data['baseList'].forEach((val, i, ar) => {
                ar[i] = this.DataProcessing.returnTreeTable(this.DataProcessing.replaceChildlValue(val, 'childList', 'children', '', ''));
        ...
        if(this.name_active_base != null && this.name_active_base != "" && this.name_active_base.length>0){
            ...
            this.name_active_base.forEach((value,index,arr)=>{
                ...
                console.log(this.name_active_base);
                //计算分类明细总面积
                this.jzzmj +=this.InputChange.CalcSize2(this.name_active_base[index],'sjmj',0);
            ...
            if(this.land_name_active_base_copy[0] == 0){

                this.land_name_active_base_copy[0] = _.cloneDeep(this.name_active_base);
                //this.land_name_active_base_copy2 = _.cloneDeep(this.name_active_base);
            ...
            console.log(this.name_active_base);
            console.log(this.land_name_active_base_copy);
        ...
        if(list != undefined && list.length > 0){
            list.forEach((value,index,arr)=>{
                ...
                if(list[index].children !=null){
                    ...
                    //if(list[index]['data']['sjmj'] == null || list[index]['data']['sjmj'] == ""){

                    // this.Tree_update(name_active_base,value,i);

                    if(list[index].parent != undefined){

                        this.InputChange.get_data(list[index].parent,'sjmj');
...
//选择name改变样式
    selectName(name,i){
        ...
        if(this.landList != undefined && this.landList.length>0){
            console.log(this.landList.length);
            console.log(this.landList);
            ...
            if(this.init_land_data['datalist']!= undefined && this.init_land_data['datalist'][this.name_active_key] != undefined && this.init_land_data['datalist'][this.name_active_key]['list']){
                this.init_name_active_base = _.cloneDeep(this.init_land_data['datalist'][this.name_active_key]['list']);
            }else{
    ...
    addland(event){
        ...
        this.name_active_base = JSON.parse(JSON.stringify(this.init_land_data['baseList']));
        console.log(this.name_active_base);
        //转换数据格式
        ...
        if( this.landList != undefined && this.landList.length>0){
            this.landList.push(new LandData('','','','','','',new Date().getTime(),'','',event,'',[],'','','','','','','','','','','','',name,'','','','','','','','',''));
        ...
        if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
            ...
            for(let i in this.add_land_data){
                if(i == this.name_active_data['id']){
                    this.add_land_data[i] = this.name_active_data;
                }
            }
        ...
        console.log(this.flxx_land_del);
        console.log(this.flxx_land_add);
        console.log(this.flxx_land_update);
    ...
    deleteSlect(){
        ...
        if(this.landList.length>0){
            ...
            if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
                ...
                this.childInfo2.forEach((value, index, arr) => {
                    ...
                    if (value['id'] == this.name_active_data['qsrId']) {
                        ...
                        delete this.listHcyAdd[value['id']]['listTdAdd'][this.name_active_data['id']];
                ...
                if(this.add_land_data[this.name_active_data['id']]){
                    delete this.add_land_data[this.name_active_data['id']];
                }
                ...
                this.landList.forEach((value, index, arr) => {
                    if (value['id'] == this.name_active_data['id']) {
                        delete this.update_land_data[index];
                    }
                });
                console.log(this.del_land_data);
            ...
            if(this.landList[0] != undefined){
                if(this.name_active_key == 0){
                    this.name_active_data = this.landList[0];
                }else if(this.name_active_key == this.landList.length){

                    this.name_active_key = this.name_active_key-1;
                    this.name_active_data = this.landList[this.name_active_key];
    ...
    Tree_update(data,now_data,i){
        ...
        if(this.landList != undefined && this.landList.length>0){
            ...
            if(this.now_data.parent != undefined){
                ...
                this.InputChange.get_data(this.now_data.parent,'sjmj');
            ...
            this.sstdjbxxId = this.name_active_data['id'];
            ...
            if(this.name_active_data.qsrId.toString().length == 32){
                ...
                if(this.name_active_data.id != undefined && this.name_active_data.id.toString().length == 32){
                    ...
                    if(this.now_data['data']['id'] != null && this.now_data['data']['id'] != ""){
                        ...
                        //如果当前分类信息值为空
                        if(this.now_data['data']['sjmj'] == null || this.now_data['data']['sjmj'] == ""){

                            //删除当前分类信息，将删除id放入flxx_land_del字段
                            this.flxx_land_del[this.now_data['data']['id']] ={id:this.now_data['data']['id']};


                            //如果之前该分类信息有更新，则删除flxx_land_update中该分类信息的字段
                            for(let i in this.flxx_land_update){

                                console.log(this.flxx_land_update[i].id);
                                console.log(this.now_data['data']['id']);
                                if(this.flxx_land_update[i].id == this.now_data['data']['id']){
                                    delete this.flxx_land_update[i];
                                    console.log(this.flxx_land_update[i]);
                                }

                            }


                            for(let i  in this.flxx_land_add){
                                console.log(this.flxx_land_add[i]['zbflId']);
                                console.log(this.now_data['data']['zbflId']);
                                if(this.flxx_land_add[i]['zbflId'] == this.now_data['data']['zbflId'] ){
                                    delete this.flxx_land_add[i];
                                }
                            }
                            ...
                            for(let i in this.flxx_land_del){

                                if(this.flxx_land_del[i]['id'] == this.now_data['data']['id']){
                                    is_del = 1;
                                }
                            }


                            //如果不在 flxx_land_del 中，则更新flxx_land_update
                            if(is_del ==0){

                                let res = this.InputChange.IsWith(this.init_name_active_base[i],this.tdlbdm,this.sstdjbxxId,now_data['data']['sjmj'],'sjmj','tdlbdm','sstdjbxxId');
                                let res2 = this.InputChange.isWithBz2(this.init_name_active_base[i],now_data,now_data['data']['bz'],'bz');

                                if(res == false || res2 == false){
                                    this.flxx_land_update[this.now_data['data']['id']] = new Array();
                                    this.flxx_land_update[this.now_data['data']['id']] = {'id':this.now_data['data']['id'],'sjmj':this.now_data['data']['sjmj'],'bz':this.now_data['data']['bz'],'tdlbdm':this.now_data['data']['tdlbdm'],'zbflId':this.now_data['data']['zbflId'],'sstdjbxxId':this.name_active_data.id};

                                }else{

                                    delete this.flxx_land_update[this.now_data['data']['id']];
                                ...
                                if(this.flxx_land_add[this.name_active_data['id']+this.now_data['data']['zbflId']]){
                                    this.flxx_land_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                                }


                                this.flxx_land_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                                this.flxx_land_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = {'sjmj':this.now_data['data']['sjmj'],'bz':this.now_data['data']['bz'],'tdlbdm':this.now_data['data']['tdlbdm'],'zbflId':this.now_data['data']['zbflId'],'sstdjbxxId':this.name_active_data.id};
                        ...
                        if((this.now_data['data']['sjmj'] == null || this.now_data['data']['sjmj'] == "") &&  (this.now_data['data']['bz'] == null || this.now_data['data']['bz'] == "")){

                            //如果 当前分类信息为空，则判断flxx_land_add中有没有该分类信息，如果有则删除
                            for(let i  in this.flxx_land_add){
                                console.log(this.flxx_land_add[i]['zbflId']);
                                console.log(this.now_data['data']['zbflId']);
                                if(this.flxx_land_add[i]['zbflId'] == this.now_data['data']['zbflId'] ){
                            ...
                            //如果 当前分类信息不为空，则将当前分类信息添加到flxx_land_add中

                            if(this.flxx_land_add[this.name_active_data['id']+this.now_data['data']['zbflId']] == undefined){
                                this.flxx_land_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                            }
                    ...
                    if((this.now_data['data']['bz'] != null && this.now_data['data']['bz'] != "") || (this.now_data['data']['sjmj'] != null && this.now_data['data']['sjmj'] != "" ) || (this.now_data['data']['sjmj'] != null && this.now_data['data']['sjmj'] != "") || (this.now_data['data']['fs'] != null && this.now_data['data']['fs'] != ""  )  ){

                        if(this.name_active_data['flbmxList'] == undefined){
                            this.name_active_data['flbmxList'] = new Array();
                        }
                        this.zbflId = now_data['data']['zbflId'];
                        this.name_active_data['flbmxList'][this.name_active_data['id']+now_data['data']['zbflId']] = {'id':now_data['data']['id'],'sjmj':now_data['data']['sjmj'],'bz':now_data['data']['bz'],'tdlbdm':now_data['data']['tdlbdm'],'zbflId':now_data['data']['zbflId'],'sstdjbxxId':this.name_active_data.id};
                ...
                this.name_active_data['flbmxList'][this.now_data['data']['zbflId']] = {'id':this.now_data['data']['id'],'sjmj':this.now_data['data']['sjmj'],'bz':this.now_data['data']['bz'],'tdlbdm':this.now_data['data']['tdlbdm'],'zbflId':this.now_data['data']['zbflId'],'sstdjbxxId':this.name_active_data.id};
    ...
    addGgmx(e){
        ...
        console.log(this.ggmx);
        this.ggmx.push(new Ggmx(this.ggmx.length==0?0:this.ggmx[this.ggmx.length - 1]['xh']+1,'','','','','','','','','','','','','','','','','','','','','','', this.tableList[0]['tdlbdm'],this.sstdjbxxId));
    ...
    delGgmx(){
        ...
        if(this.ggmx && this.ggmx.length >0){
            ...
            console.log(this.ggmx);
            console.log(this.flxx_land_add);
            // console.log(this.list_land_ggmx_del);
            // console.log(this.list_land_ggmx_edit);
            // console.log(this.list_land_ggmx_add);
    ...
    calc(e,now_data){
        ...
        if(e.key == 'Enter'){
            this.InputChange.calc(now_data,'sjmj');
        }
    ...
    searchList(searchKeyword){
        ...
        if(this.landList != undefined && this.landList.length>0){
            this.name_active_base.forEach((value,index,arr)=>{
                ...
                this.InputChange.delSearchRsut(this.land_name_active_base_copy[0][index],'searchRsut');
...
//==================下面下拉列表 ================================
    ...
    eventQsr(e){
        ...
        this.selectName(this.name_active_data,this.name_active_key);
        this.landList[this.name_active_key] = this.name_active_data;
        ...
        if(this.name_active_data.qsrId.toString().length == 32){
            ...
            if(this.init_land_data['datalist'][this.name_active_key] != undefined && this.init_land_data['datalist'][this.name_active_key]['id']==this.name_active_data.id ){

                delete this.update_land_data[this.name_active_key];

                this.del_land_data[this.name_active_key] = ({'id':this.init_land_data['datalist'][this.name_active_key]['id']});
                ...
                for(let i in this.flxx_land_add){
                    console.log(this.flxx_land_add[i]['sstdjbxxId']);
                    console.log(this.name_active_data['id']);
                    if(this.flxx_land_add[i]['sstdjbxxId'] ==  this.name_active_data['id']){
                        delete this.flxx_land_add[i];
                    }
                }
                ...
                for(let i in this.add_ggmx_data){
                    console.log(this.add_ggmx_data[i]['sstdjbxxId']);
                    console.log(this.name_active_data['id']);
                    if(this.add_ggmx_data[i]['sstdjbxxId'] ==  this.name_active_data['id']){
                        delete this.add_ggmx_data[i];
                    }
                }
            ...
            if(this.listHcyAdd[this.name_active_data.qsrId]['listTdAdd'][this.name_active_data.id]){
                delete this.listHcyAdd[this.name_active_data.qsrId]['listTdAdd'][this.name_active_data.id];
        ...
        if(e.toString().length == 32){
            ...
            //this.del_land_data.push({'id':temp_qsrid});
            console.log(this.add_land_data);
            ...
            for(let i in res2){
                ...
                if(this.add_land_data[this.name_active_data['id']]['flbmxList'] == null){
                    this.add_land_data[this.name_active_data['id']]['flbmxList'] = new Array();
                }



                if(this.add_land_data[this.name_active_data['id']]['flbmxList'][i]){
                    this.add_land_data[this.name_active_data['id']]['flbmxList'][i] = new Array();
                }

                this.add_land_data[this.name_active_data['id']]['flbmxList'][i] = res2[i];

            }

            this.name_active_data = this.add_land_data[this.name_active_data['id']];
            ...
            if(this.name_active_data['flbmxList'] != undefined && this.name_active_data['flbmxList'].length>0){
                ...
                for(let i in this.name_active_data['flbmxList']){
                    if((this.name_active_data['flbmxList'][i]['sjmj'] == "" || this.name_active_data['flbmxList'][i]['sjmj'] == null) && (this.name_active_data['flbmxList'][i]['bz'] == "" && this.name_active_data['flbmxList'][i]['bz'] == null)){
...
//是否有无房产
    ...
    showZydlBlock() {
        ...
        if(this.type != 'view'){
            ...
            if (!this.zydlTableList) {
                ...
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_TDJBXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        this.zydlTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_TDJBXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
    ...
    showtdxzBlock() {
        ...
        if(this.type != 'view'){
            this.isShowTdxz = this.isShowTdxz ? false : true;


            if (!this.TdxzTableList) {

                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_TDJBXX&column=TDXZDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        this.TdxzTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_TDJBXX&column=TDXZDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        console.log(res['returnObject']);
                        this.TdxzTableList = res['returnObject'];
                    });
    ...
    getChildtdxz(event) {
        console.log(event);
        this.isShowTdxz = false;
        this.name_active_data['tdxzmc'] = event.mc;
        this.name_active_data['tdxzdm'] = event.dm;
...
//行政区划代码
    ...
    getChildytjjg(event) {
        ...
        this.ggmx[this.GgmxIndex]['tdlbdm'] = event.node.tdlbdm;
    ...
    ToggleFlxxAdd(arr,e,sstdjbxxId){
        for (let item of arr) {
            console.log(item);
            if (item.children) {

                this.ToggleFlxxAdd(item.children,e,sstdjbxxId);
            } else {

                if((item['data']['sjmj'] != null && item['data']['sjmj'] != "" )){

                    this.toggle_flxx_add[sstdjbxxId+item.data['zbflId']] = new Array();
                    this.toggle_flxx_add[sstdjbxxId+item.data['zbflId']] = {'sjmj':item['data']['sjmj'],'bz':item['data']['bz'],'tdlbdm':item['data']['tdlbdm'],'zbflId':item['data']['zbflId'],'qsrId':e,'sstdjbxxId':sstdjbxxId};
    ...
    ToggleFlxxAdd2(arr,e,sstdjbxxId){
        this.toggle_flxx_add = new Array();

        arr.forEach((value,index,arr)=>{
            this.ToggleFlxxAdd(arr[index],e,sstdjbxxId);
