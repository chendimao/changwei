...
@Component({
  selector: 'app-sheshi',
  templateUrl: './sheshi.component.html',
    styleUrls: ['../children.css']
})
export class SheshiComponent implements OnInit {
    public hcy: any;
    public selectListHzgx = new Array;
    public tableList = Array();
    public sheshiList: any;
    types: SelectItem[];
    ...
    public del_sheshi_data; //  删除的数据
    public init_sheshi_data; //  初始化的数据
    public update_sheshi_data = new Array(); //  对比后修改的数据
    public add_sheshi_data = new Array(); //  新增的数据
    ...
    @ViewChildren('defaultsheshi') defaultsheshi: QueryList<ElementRef>;
    public isShowsslb;
    public sslbTreeList;
    public sslbTableList;
    ...
    ngAfterViewInit(): void {
        ...
        if (this.tableList.length > 0) {
            ...
            this.person.valueChanges.subscribe(data => {
                    ...
                    console.log(this.add_sheshi_data);
                    console.log(this.update_sheshi_data);
                    console.log(this.init_sheshi_data);
    ...
    ngOnInit() {
        ...
        //
        //         this.add_sheshi_data[index] = value;
        //
        //     })
        // }
    ...
    showZydlBlock(index) {
        ...
        if (this.type != "view") {
            ...
            if (!this.zydlTableList) {
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_SSXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        console.log(res);
                        this.zydlTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_SSXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
    ...
    showDcfwBlock(index) {
        ...
        if (this.type != "view") {
            ...
            if (!this.dcfwTableList) {
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_SSXX&column=DCFWDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        console.log(res);
    ...
    showsslbBlock(index) {
        ...
        if (this.type != "view") {
            this.isShowsslb = this.isShowsslb ? false : true;
            if (!this.dcfwTableList) {
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_SSXX&column=SSLBDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        console.log(res);
                        this.sslbTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_SSXX&column=SSLBDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        console.log(res['returnObject']);
                        this.sslbTableList = res['returnObject'];
                    });
            }
        }
    }
    ...
    addsheshi(): void {
        ...
        this.selectsheshi(this.hcy,this.tableList.length-1);
        // if(this.childInfo2 && this.childInfo2.length>0 && this.childInfo2[0]['id'].toString().length == 32) {
        //
        // }else{
        //     this.add_sheshi_data.push(this.tableList[this.hcy_count-1]);
        // }
        //
        //
        //
        ...
        this.add_sheshi_data[this.hcy['id']]=this.hcy;
    ...
    delsheshi() {
        ...
        if (this.tableList.length > 0) {
            ...
            if (this.hcy['id'] != undefined && this.hcy['id'].toString().length == 32) {
                ...
                this.del_sheshi_data.push({'id': this.hcy['id']});

                //  如果删除的户成员在修改列表中存在，则从修改列表中删除
                this.tableList.forEach((value, index, arr) => {
                    if (value['id'] == this.hcy['id']) {
                        delete this.update_sheshi_data[index];
                    }
                });
                console.log(this.update_sheshi_data);
                console.log(this.del_sheshi_data);
                ...
                delete this.add_sheshi_data[this.hcy['id']];
                console.log(this.add_sheshi_data);
            ...
            if (this.tableList[0] != undefined && this.name_active_key > 0) {
                this.selectsheshi(this.tableList[this.name_active_key - 1], this.name_active_key - 1);
                this.hcy_count = this.hcy_count - 1;

            } else if (this.tableList[0] != undefined && this.name_active_key == 0) {

                this.selectsheshi(this.tableList[this.name_active_key], this.name_active_key);
                this.hcy_count = this.hcy_count - 1;
    ...
    eventQsr(event){
        ...
        if(this.hcy.qsrId){
            ...
            if(event!= -1 && this.hcy.qsrId.toString().length == 32 && this.hcy.id.toString().length == 32){
                ...
                //删除update_sheshi_data中 上一个坟墓的数据
                if(this.update_sheshi_data.length>0){
                    this.update_sheshi_data.forEach((value,index,arr)=>{

                        if(value['id'] == this.hcy.id){
                            this.update_sheshi_data.splice(index,1);
                        }

                    })
                    console.log(this.update_sheshi_data[0]);
                    console.log(this.update_sheshi_data);
                }
        ...
        if(event.toString().length == 32){
            ...
            console.log(this.hcy.qsrId);
            this.add_sheshi_data.forEach((value,index,arr)=>{

                if(value['qsrId'] == this.hcy.qsrId){
                    delete this.add_sheshi_data[index];
                }
