...
export class WaterComponent implements OnInit {
    ...
    public del_water_data = new Array(); //删除的数据
    public init_water_data; //初始化的数据
    public update_water_data; //对比后修改的数据
    public add_water_data =new Array(); //新增的数据
    ...
    public bj_water_data; //初始化数据标记
    public bj_water_new_data; //初始化新增数据标记
    public flxx_water_update = new Array(); //分类信息
    public flxx_water_add = new Array(); //分类信息新增
    public flxx_water_del = new Array(); //分类信息删除
    ...
    public water_name_active_base_copy;
    public water_name_active_base_copy2;
    public szxzqugldm;
...
// 订阅表单值改变事件
            this.person2.valueChanges.subscribe(data => {
                    ...
                    if(this.waterList.length>0){
                        if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
                            ...
                            this.listHcyAdd = this.InputChange.get_add_change(this.childInfo2,this.name_active_data,this.listHcyAdd,'listXxzxAdd');
                            ...
                            let res =  this.InputChange.get_value_change(this.name_active_data,this.name_active_key,this.init_water_data,this.update_water_data,this.add_water_data);
    ...
    ngOnInit() {
        ...
        //
        // let oninit_res = this.TabCommon.Oninit(this.data,this.name_active_data,this.waterList,this.name_active_base,this.bj_water_data);
        //
        ...
        // this.waterList = oninit_res.List;
        // this.bj_water_data = oninit_res.bj_data;

        if(this.data['returnObject']['datalist']){
            if(this.data['returnObject']['datalist'][0] !=undefined ){


                this.name_active_data = [ new waterData('','','','','','',new Date().getTime(),'','','','',[],'','','','','','','','','','','','','','','','','','','','','','') ];

                this.waterList = this.data['returnObject']['datalist'];  //户成员信息
                console.log(this.waterList);
                this.name_active_data = this.waterList[0];   //默认选中第一个户成员
                ...
                if(this.init_water_data['datalist'] != undefined && this.init_water_data['datalist'].length>0){

                    this.init_name_active_base = this.init_water_data['datalist'][0]['list'];
                }
...
// =============== end 规格明细  =======================
        ...
        if(this.bj_water_data == 1) {
            this.bj_water_data = 0;
            if(this.data['returnObject']['datalist'] && this.data['returnObject']['datalist'].length>0){

                this.waterList.forEach((value,index,arr)=>{
                    console.log(value);
            ...
            if(this.init_water_data['datalist'] != undefined && this.init_water_data['datalist'].length >0){
                this.init_water_data['datalist'].forEach((value,index,arr)=>{
                    value.list.forEach((val,i,ar)=>{
                        ar[i] = this.DataProcessing.returnTreeTable(this.DataProcessing.replaceChildlValue(val, 'childList', 'children', '', ''));
            ...
            this.init_water_data['baseList'].forEach((val, i, ar) => {
                ar[i] = this.DataProcessing.returnTreeTable(this.DataProcessing.replaceChildlValue(val, 'childList', 'children', '', ''));
        ...
        if(this.name_active_base != null && this.name_active_base != "" && this.name_active_base.length>0){
            ...
            if(this.water_name_active_base_copy[0] == 0){

                this.water_name_active_base_copy[0] = _.cloneDeep(this.name_active_base);
                //this.water_name_active_base_copy2 = _.cloneDeep(this.name_active_base);
            ...
            console.log(this.name_active_base);
            console.log(this.water_name_active_base_copy);
...
//选择name改变样式
    selectName(name,i){
        ...
        if(this.waterList != undefined && this.waterList.length>0){
            console.log(this.waterList.length);
            console.log(this.waterList);
            ...
            if(this.init_water_data['datalist']!= undefined && this.init_water_data['datalist'][this.name_active_key] != undefined && this.init_water_data['datalist'][this.name_active_key]['list']){
                this.init_name_active_base = _.cloneDeep(this.init_water_data['datalist'][this.name_active_key]['list']);
            }else{
    ...
    addwater(event){
        ...
        this.name_active_base = JSON.parse(JSON.stringify(this.init_water_data['baseList']));
        console.log(this.name_active_base);
        //转换数据格式
        ...
        if( this.waterList != undefined && this.waterList.length>0){
            this.waterList.push(new waterData('','','','','','',new Date().getTime(),'','',event,'',[],'','','','','','','','','','','','',name,'','','','','','','','',this.ssxxzxjbxxId));
            this.name_active_data = this.waterList[this.waterList.length-1];
            this.name_active_key = this.waterList.length-1;
            this.name_active_data['list'] = this.name_active_base;


        }else{
            this.waterList.push(new waterData('','','','','','',new Date().getTime(),'','',event,'',[],'','','','','','','','','','','','',name,'','','','','','','','',this.ssxxzxjbxxId));
            this.name_active_data = this.waterList[0];
            this.name_active_data['list'] = this.name_active_base;
        ...
        if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
            ...
            for(let i in this.add_water_data){
                if(i == this.name_active_data['id']){
                    this.add_water_data[i] = this.name_active_data;
                }
            }
        ...
        console.log(this.flxx_water_del);
        console.log(this.flxx_water_add);
        console.log(this.flxx_water_update);
    ...
    deleteSlect(){
        ...
        if(this.waterList.length>0){
            ...
            if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
                ...
                this.childInfo2.forEach((value, index, arr) => {
                    ...
                    if (value['id'] == this.name_active_data['qsrId']) {
                        ...
                        delete this.listHcyAdd[value['id']]['listXxzxAdd'][this.name_active_data['id']];
                ...
                if(this.add_water_data[this.name_active_data['id']]){
                    delete this.add_water_data[this.name_active_data['id']];
                }

                this.del_water_data.push({'id':this.name_active_data['id']});

                //如果删除的户成员在修改列表中存在，则从修改列表中删除

                this.waterList.forEach((value, index, arr) => {
                    if (value['id'] == this.name_active_data['id']) {
                        delete this.update_water_data[index];
                    }
                });
                console.log(this.del_water_data);
            ...
            //
            // this.waterList.splice(this.name_active_key,1);
            // if(this.waterList[0] != undefined && this.waterList.length>0  ){
            //
            //     if(this.name_active_key == 0){
            //         this.name_active_data = this.waterList[this.name_active_key+1];
            ...
            //
            //
            // }else if(this.waterList[0] != undefined && this.name_active_key==0){
            //
            //     this.name_active_data = this.waterList[this.name_active_key+1];
            //     this.name_active_key = this.name_active_key+1;
            //
            ...
            if(this.waterList[0] != undefined){
                if(this.name_active_key == 0){
                    this.name_active_data = this.waterList[0];
                }else if(this.name_active_key == this.waterList.length){

                    this.name_active_key = this.name_active_key-1;
                    this.name_active_data = this.waterList[this.name_active_key];
    ...
    Tree_update(data,now_data,i){
        ...
        if(this.waterList != undefined && this.waterList.length>0){
            ...
            this.ssxxzxjbxxId = this.name_active_data['id'];
            ...
            if(this.name_active_data.qsrId.toString().length == 32){
                ...
                if(this.name_active_data.id != undefined && this.name_active_data.id.toString().length == 32){
                    ...
                    if(this.now_data['data']['id'] != null && this.now_data['data']['id'] != ""){
                        ...
                        if((this.now_data['data']['sl'] == null || this.now_data['data']['sl']== "")){
                            ...
                            //删除当前分类信息，将删除id放入flxx_water_del字段
                            this.flxx_water_del[this.now_data['data']['id']] ={id:this.now_data['data']['id']};


                            //如果之前该分类信息有更新，则删除flxx_water_update中该分类信息的字段
                            for(let i in this.flxx_water_update){

                                console.log(this.flxx_water_update[i].id);
                                console.log(this.now_data['data']['id']);
                                if(this.flxx_water_update[i].id == this.now_data['data']['id']){
                                    delete this.flxx_water_update[i];
                                    console.log(this.flxx_water_update[i]);
                                }

                            }


                            for(let i  in this.flxx_water_add){
                                console.log(this.flxx_water_add[i]['zbflId']);
                                console.log(this.now_data['data']['zbflId']);
                                if(this.flxx_water_add[i]['zbflId'] == this.now_data['data']['zbflId'] ){
                                    delete this.flxx_water_add[i];
                                }
                            }
                            ...
                            for(let i in this.flxx_water_del){

                                if(this.flxx_water_del[i]['id'] == this.now_data['data']['id']){
                                    is_del = 1;
                                }
                            }


                            //如果不在 flxx_water_del 中，则更新flxx_water_update
                            if(is_del ==0){

                                let res = this.InputChange.IsWith(this.init_name_active_base[i],this.sslb,this.ssxxzxjbxxId,now_data['data']['sl'],'sl','sslb','ssxxzxjbxxId');
                                let res2 = this.InputChange.isWithBz2(this.init_name_active_base[i],now_data,now_data['data']['bz'],'bz');

                                if(res == false || res2 == false){
                                    this.flxx_water_update[this.now_data['data']['id']] = new Array();
                                    this.flxx_water_update[this.now_data['data']['id']] = {'id':this.now_data['data']['id'],'sl':this.now_data['data']['sl'],'bz':this.now_data['data']['bz'],'sslb':this.now_data['data']['sslb'],'zbflId':this.now_data['data']['zbflId'],'ssxxzxjbxxId':this.name_active_data.id};

                                }else{

                                    delete this.flxx_water_update[this.now_data['data']['id']];

                                }
                            }else{

                                //否则放入 flxx_water_add中

                                if(this.flxx_water_add[this.name_active_data['id']+this.now_data['data']['zbflId']]){
                                    this.flxx_water_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                                }


                                this.flxx_water_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                                this.flxx_water_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = {'sl':this.now_data['data']['sl'],'bz':this.now_data['data']['bz'],'sslb':this.now_data['data']['sslb'],'zbflId':this.now_data['data']['zbflId'],'ssxxzxjbxxId':this.name_active_data.id};
                        ...
                        if((this.now_data['data']['sl'] == null || this.now_data['data']['sl'] == "") &&  (this.now_data['data']['bz'] == null || this.now_data['data']['bz'] == "")){
                            ...
                            //如果 当前分类信息为空，则判断flxx_water_add中有没有该分类信息，如果有则删除
                            for(let i  in this.flxx_water_add){
                                console.log(this.flxx_water_add[i]['zbflId']);
                                console.log(this.now_data['data']['zbflId']);
                                if(this.flxx_water_add[i]['zbflId'] == this.now_data['data']['zbflId'] ){
                            ...
                            //如果 当前分类信息不为空，则将当前分类信息添加到flxx_water_add中

                            if(this.flxx_water_add[this.name_active_data['id']+this.now_data['data']['zbflId']] == undefined){
                                this.flxx_water_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                            }
                    ...
                    if((this.now_data['data']['bz'] != null && this.now_data['data']['bz'] != "") || (this.now_data['data']['sl'] != null && this.now_data['data']['sl'] != "" ) || (this.now_data['data']['sl'] != null && this.now_data['data']['sl'] != "") || (this.now_data['data']['fs'] != null && this.now_data['data']['fs'] != ""  )  ){
                        ...
                        this.name_active_data['flbmxList'][this.name_active_data['id']+now_data['data']['zbflId']] = {'id':now_data['data']['id'],'sl':now_data['data']['sl'],'bz':now_data['data']['bz'],'sslb':now_data['data']['sslb'],'zbflId':now_data['data']['zbflId'],'ssxxzxjbxxId':this.name_active_data.id};
                ...
                this.name_active_data['flbmxList'][this.now_data['data']['zbflId']] = {'id':this.now_data['data']['id'],'sl':this.now_data['data']['sl'],'bz':this.now_data['data']['bz'],'sslb':this.now_data['data']['sslb'],'zbflId':this.now_data['data']['zbflId'],'ssxxzxjbxxId':this.name_active_data.id};
            ...
            console.log(this.add_water_data);
            console.log(this.ssxxzxjbxxId);
            console.log(this.name_active_base);

            console.log(this.isShow);
            let res = this.InputChange.showCheck3(this.name_active_base,'sslb');
            console.log(res);



                this.water_name_active_base_copy[0][i] = this.InputChange.showCheck4(this.water_name_active_base_copy[0][i],res,'sslb');
    ...
    TreeTableClick(e,data,now_data,i){
        ...
        this.sslb = e.node.data.sslb;
        console.log(this.sslb);
    ...
    delGgmx(){
        ...
        if(this.ggmx && this.ggmx.length >0){
            ...
            console.log(this.ggmx);
            console.log(this.flxx_water_add);
            // console.log(this.list_water_ggmx_del);
            // console.log(this.list_water_ggmx_edit);
            // console.log(this.list_water_ggmx_add);
    ...
    selectedTypea3(): void {
        ...
        name_active_base_copy.forEach((value,index,arr)=>{
            ...
            value = this.DataProcessing.returnTreeGgmx(value,'sslb');
            value.forEach((v,i,a)=>{
    ...
    searchList(searchKeyword){
        ...
        if(this.waterList != undefined && this.waterList.length>0){
            this.name_active_base.forEach((value,index,arr)=>{
                ...
                this.InputChange.delSearchRsut(this.water_name_active_base_copy[0][index],'searchRsut');
...
//==================下面下拉列表 ================================
    ...
    eventQsr(e){
        ...
        this.selectName(this.name_active_data,this.name_active_key);
        this.waterList[this.name_active_key] = this.name_active_data;
        ...
        if(this.name_active_data.qsrId.toString().length == 32){
            ...
            if(this.init_water_data['datalist'][this.name_active_key] != undefined && this.init_water_data['datalist'][this.name_active_key]['id']==this.name_active_data.id ){

                delete this.update_water_data[this.name_active_key];

                this.del_water_data[this.name_active_key] = ({'id':this.init_water_data['datalist'][this.name_active_key]['id']});
                ...
                for(let i in this.flxx_water_add){
                    console.log(this.flxx_water_add[i]['ssxxzxjbxxId']);
                    console.log(this.name_active_data['id']);
                    if(this.flxx_water_add[i]['ssxxzxjbxxId'] ==  this.name_active_data['id']){
                        delete this.flxx_water_add[i];
                    }
                }
                ...
                for(let i in this.add_ggmx_data){
                    console.log(this.add_ggmx_data[i]['ssxxzxjbxxId']);
                    console.log(this.name_active_data['id']);
                    if(this.add_ggmx_data[i]['ssxxzxjbxxId'] ==  this.name_active_data['id']){
                        delete this.add_ggmx_data[i];
                    }
                }
            ...
            if(this.listHcyAdd[this.name_active_data.qsrId]['listXxzxAdd'][this.name_active_data.id]){
                delete this.listHcyAdd[this.name_active_data.qsrId]['listXxzxAdd'][this.name_active_data.id];
        ...
        if(e.toString().length == 32){
            ...
            //this.del_water_data.push({'id':temp_qsrid});
            console.log(this.add_water_data);
            ...
            for(let i in res2){
                ...
                if(this.add_water_data[this.name_active_data['id']]['flbmxList'] == null){
                    this.add_water_data[this.name_active_data['id']]['flbmxList'] = new Array();
                }



                if(this.add_water_data[this.name_active_data['id']]['flbmxList'][i]){
                    this.add_water_data[this.name_active_data['id']]['flbmxList'][i] = new Array();
                }

                this.add_water_data[this.name_active_data['id']]['flbmxList'][i] = res2[i];

            }

            this.name_active_data = this.add_water_data[this.name_active_data['id']];
    ...
    showZydlBlock() {
        ...
        if(this.type != 'view'){
            ...
            if (!this.zydlTableList) {
                ...
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_XXZXJBXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        this.zydlTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_XXZXJBXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
    ...
    showDcfwBlock() {
        ...
        if(this.type != 'view'){
            ...
            if (!this.dcfwTableList) {
                ...
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_XXZXJBXX&column=DCFWDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        this.dcfwTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_XXZXJBXX&column=DCFWDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        console.log(res['returnObject']);
...
//行政区划代码
    ...
    showAll(i){
        ...
        if(i){
            ...
            for(let i in this.water_name_active_base_copy[0]){
    ...
    getChildytjjg(event) {
        ...
        this.ggmx[this.GgmxIndex]['xxzxlbdm'] = event.node.sslb;
    ...
    ToggleFlxxAdd(arr,e,ssxxzxjbxxId){
        for (let item of arr) {
            console.log(item);
            if (item.children) {

                this.ToggleFlxxAdd(item.children,e,ssxxzxjbxxId);
            } else {

                if((item['data']['sl'] != null && item['data']['sl'] != "" )){

                    this.toggle_flxx_add[ssxxzxjbxxId+item.data['zbflId']] = new Array();
                    this.toggle_flxx_add[ssxxzxjbxxId+item.data['zbflId']] = {'sl':item['data']['sl'],'bz':item['data']['bz'],'sslb':item['data']['sslb'],'zbflId':item['data']['zbflId'],'qsrId':e,'ssxxzxjbxxId':ssxxzxjbxxId};
    ...
    ToggleFlxxAdd2(arr,e,ssxxzxjbxxId){
        this.toggle_flxx_add = new Array();

        arr.forEach((value,index,arr)=>{
            this.ToggleFlxxAdd(arr[index],e,ssxxzxjbxxId);
