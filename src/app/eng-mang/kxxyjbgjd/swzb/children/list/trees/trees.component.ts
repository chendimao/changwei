...
export class TreesComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        //  专业大类 下拉列表
        this.selectList.getSelectList('B_LXGMJBXX', 'ZYDLDM', this.ssgcdm, this.ssxzqhdm).then(data => {
            this.zydl_list = data;
        });
        //  房屋性质 下拉列表
    ...
    getChildEvent1(event){
        ...
        this.name_active_data['list'] = JSON.parse(JSON.stringify(this.init_trees_data['baseList']));
        ...
        if((typeof this.treesList) != 'undefined' && this.treesList.length>0){
            this.treesList.push(new TreesData('','','','','','','','','',event,'','','','','',0,'','','','','','','','',name,'','',''));
    ...
    deleteSlect(){
        ...
        // if(this.treesList[0] != undefined && this.name_active_key>0){
        console.log(this.name_active_data);
    ...
    dcfw_show(){
        ...
        if(this.dcfw_list_is_show == 0){
            ...
            this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_LXGMJBXX&column=DCFWDM&gcdm=${this.qshflId.ssgcdm}&xzqhdm=${this.qshflId.ssxzqhdm}`).then((data)=>{
