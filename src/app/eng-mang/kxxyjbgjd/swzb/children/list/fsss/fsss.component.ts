...
export class FsssComponent implements OnInit {
    ...
    public del_fsss_data = new Array(); //删除的数据
    public init_fsss_data; //初始化的数据
    public update_fsss_data; //对比后修改的数据
    public add_fsss_data =new Array(); //新增的数据
    ...
    public bj_fsss_data; //初始化数据标记
    public bj_fsss_new_data; //初始化新增数据标记
    public flxx_fsss_update = new Array(); //分类信息
    public flxx_fsss_add = new Array(); //分类信息新增
    public flxx_fsss_del = new Array(); //分类信息删除
    ...
    public fsss_name_active_base_copy;
    public szxzqugldm;
...
// 订阅表单值改变事件
            this.person2.valueChanges.debounceTime(500).subscribe(data => {
                    ...
                    if(this.fsssList.length>0){
                        if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
                            ...
                            let res =  this.InputChange.get_value_change(this.name_active_data,this.name_active_key,this.init_fsss_data,this.update_fsss_data,this.add_fsss_data);

                            this.add_fsss_data = res.add_data;
                            this.update_fsss_data = res.update_data;
                        }
                    }
    ...
    ngOnInit() {
        ...
        //
        // let oninit_res = this.TabCommon.Oninit(this.data,this.name_active_data,this.fsssList,this.name_active_base,this.bj_fsss_data);
        //
        ...
        // this.fsssList = oninit_res.List;
        // this.bj_fsss_data = oninit_res.bj_data;

        if(this.data['returnObject']['datalist']){
            if(this.data['returnObject']['datalist'][0] !=undefined ){


                this.name_active_data = [ new fsssData('','','','','','',new Date().getTime(),'','','','',[],'','','','','','','','','','','','','','','','','','','','','','') ];

                this.fsssList = this.data['returnObject']['datalist'];  //户成员信息
                console.log(this.fsssList);
                this.name_active_data = this.fsssList[0];   //默认选中第一个户成员
                ...
                if(this.init_fsss_data['datalist'] != undefined && this.init_fsss_data['datalist'].length>0){

                    this.init_name_active_base = this.init_fsss_data['datalist'][0]['list'];
                }
...
// =============== end 规格明细  =======================
        ...
        if(this.bj_fsss_data == 1) {
            ...
            if(this.init_fsss_data['datalist'] != undefined && this.init_fsss_data['datalist'].length >0){
                this.init_fsss_data['datalist'].forEach((value,index,arr)=>{
                    value.list.forEach((val,i,ar)=>{
                        ar[i] = this.DataProcessing.returnTreeTable(this.DataProcessing.replaceChildlValue(val, 'childList', 'children', '', ''));
            ...
            this.init_fsss_data['baseList'].forEach((val, i, ar) => {
                ar[i] = this.DataProcessing.returnTreeTable(this.DataProcessing.replaceChildlValue(val, 'childList', 'children', '', ''));
        ...
        if(this.name_active_base != null && this.name_active_base != "" && this.name_active_base.length>0){
            ...
            if(this.fsss_name_active_base_copy[0] == 0){

                this.fsss_name_active_base_copy[0] = _.cloneDeep(this.name_active_base);
                //this.fsss_name_active_base_copy2 = _.cloneDeep(this.name_active_base);
                console.log(this.fsss_name_active_base_copy[0]);

            }else{
                this.name_active_base = _.cloneDeep(this.fsss_name_active_base_copy[0]);
                console.log(this.fsss_name_active_base_copy[0]);
            ...
            console.log(this.name_active_base);
            console.log(this.fsss_name_active_base_copy);
...
//选择name改变样式
    selectName(name,i){
        ...
        if(this.fsssList != undefined && this.fsssList.length>0){
            console.log(this.fsssList.length);
            console.log(this.fsssList);
            ...
            if(this.init_fsss_data['datalist']!= undefined && this.init_fsss_data['datalist'][this.name_active_key] != undefined && this.init_fsss_data['datalist'][this.name_active_key]['list']){
                this.init_name_active_base = _.cloneDeep(this.init_fsss_data['datalist'][this.name_active_key]['list']);
            }else{
    ...
    addfsss(event){
        ...
        this.name_active_base = JSON.parse(JSON.stringify(this.init_fsss_data['baseList']));
        console.log(this.name_active_base);
        //转换数据格式
        ...
        if( this.fsssList != undefined && this.fsssList.length>0){
            this.fsssList.push(new fsssData('','','','','','',new Date().getTime(),'','',event,'',[],'','','','','','','','','','','','',name,'','','','','','','','',''));
            this.name_active_data = this.fsssList[this.fsssList.length-1];
            this.name_active_data['list'] = this.name_active_base;

            this.name_active_key = this.fsssList.length-1;

        }else{
            this.fsssList.push(new fsssData('','','','','','',new Date().getTime(),'','',event,'',[],'','','','','','','','','','','','',name,'','','','','','','','',''));
            this.name_active_data = this.fsssList[0];
            this.name_active_data['list'] = this.name_active_base;
        ...
        if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
            ...
            for(let i in this.add_fsss_data){
                if(i == this.name_active_data['id']){
                    this.add_fsss_data[i] = this.name_active_data;
                }
            }
        ...
        console.log(this.flxx_fsss_del);
        console.log(this.flxx_fsss_add);
        console.log(this.flxx_fsss_update);
    ...
    deleteSlect(){
        ...
        if(this.fsssList.length>0){
            ...
            if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
                ...
                this.childInfo2.forEach((value, index, arr) => {
                    ...
                    if (value['id'] == this.name_active_data['qsrId']) {
                        ...
                        delete this.listHcyAdd[value['id']]['listFsssAdd'][this.name_active_data['id']];
                ...
                if(this.add_fsss_data[this.name_active_data['id']]){
                    delete this.add_fsss_data[this.name_active_data['id']];
                }

                this.del_fsss_data.push({'id':this.name_active_data['id']});

                //如果删除的户成员在修改列表中存在，则从修改列表中删除

                this.fsssList.forEach((value, index, arr) => {
                    if (value['id'] == this.name_active_data['id']) {
                        delete this.update_fsss_data[index];
                    }
                });
                console.log(this.del_fsss_data);
            ...
            //
            // this.fsssList.splice(this.name_active_key,1);
            // if(this.fsssList[0] != undefined && this.fsssList.length>0  ){
            //
            //     if(this.name_active_key == 0){
            //         this.name_active_data = this.fsssList[this.name_active_key+1];
            ...
            //
            //
            // }else if(this.fsssList[0] != undefined && this.name_active_key==0){
            //
            //     this.name_active_data = this.fsssList[this.name_active_key+1];
            //     this.name_active_key = this.name_active_key+1;
            //
            ...
            if(this.fsssList[0] != undefined){
                if(this.name_active_key == 0){
                    this.name_active_data = this.fsssList[0];
                }else if(this.name_active_key == this.fsssList.length){

                    this.name_active_key = this.name_active_key-1;
                    this.name_active_data = this.fsssList[this.name_active_key];
    ...
    Tree_update(data,now_data,i){
        ...
        if(this.fsssList != undefined && this.fsssList.length>0){
            ...
            this.ssfsssjbxxId = this.name_active_data['id'];
            ...
            if(this.name_active_data.qsrId.toString().length == 32){
                ...
                if(this.name_active_data.id != undefined && this.name_active_data.id.toString().length == 32){
                    ...
                    if(this.now_data['data']['id'] != null && this.now_data['data']['id'] != ""){
                        ...
                        if((this.now_data['data']['sl'] == null || this.now_data['data']['sl'] == "")){
                            ...
                            //删除当前分类信息，将删除id放入flxx_fsss_del字段
                            this.flxx_fsss_del[this.now_data['data']['id']] ={id:this.now_data['data']['id']};


                            //如果之前该分类信息有更新，则删除flxx_fsss_update中该分类信息的字段
                            for(let i in this.flxx_fsss_update){

                                console.log(this.flxx_fsss_update[i].id);
                                console.log(this.now_data['data']['id']);
                                if(this.flxx_fsss_update[i].id == this.now_data['data']['id']){
                                    delete this.flxx_fsss_update[i];
                                    console.log(this.flxx_fsss_update[i]);
                                }

                            }


                            for(let i  in this.flxx_fsss_add){
                                console.log(this.flxx_fsss_add[i]['zbflId']);
                                console.log(this.now_data['data']['zbflId']);
                                if(this.flxx_fsss_add[i]['zbflId'] == this.now_data['data']['zbflId'] ){
                                    delete this.flxx_fsss_add[i];
                                }
                            }
                            ...
                            for(let i in this.flxx_fsss_del){

                                if(this.flxx_fsss_del[i]['id'] == this.now_data['data']['id']){
                                    is_del = 1;
                                }
                            }


                            //如果不在 flxx_fsss_del 中，则更新flxx_fsss_update
                            if(is_del ==0){


                                let res = this.InputChange.IsWith(this.init_name_active_base[i],this.fssslbdm,this.ssfsssjbxxId,now_data['data']['sl'],'sl','fssslbdm','ssfsssjbxxId');
                                let res2 = this.InputChange.isWithBz2(this.init_name_active_base[i],now_data,now_data['data']['bz'],'bz');

                                if(res == false || res2 == false){
                                    this.flxx_fsss_update[this.now_data['data']['id']] = new Array();
                                    this.flxx_fsss_update[this.now_data['data']['id']] = {'id':this.now_data['data']['id'],'sl':this.now_data['data']['sl'],'bz':this.now_data['data']['bz'],'fssslbdm':this.now_data['data']['fssslbdm'],'zbflId':this.now_data['data']['zbflId'],'ssfsssjbxxId':this.name_active_data.id};

                                }else{

                                    delete this.flxx_fsss_update[this.now_data['data']['id']];
                                ...
                                if(this.flxx_fsss_add[this.name_active_data['id']+this.now_data['data']['zbflId']]){
                                    this.flxx_fsss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                                }


                                this.flxx_fsss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                                this.flxx_fsss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = {'sl':this.now_data['data']['sl'],'bz':this.now_data['data']['bz'],'fssslbdm':this.now_data['data']['fssslbdm'],'zbflId':this.now_data['data']['zbflId'],'ssfsssjbxxId':this.name_active_data.id};
                        ...
                        if((this.now_data['data']['sl'] == null || this.now_data['data']['sl'] == "") &&  (this.now_data['data']['bz'] == null || this.now_data['data']['bz'] == "")){
                            ...
                            //如果 当前分类信息为空，则判断flxx_fsss_add中有没有该分类信息，如果有则删除
                            for(let i  in this.flxx_fsss_add){
                                console.log(this.flxx_fsss_add[i]['zbflId']);
                                console.log(this.now_data['data']['zbflId']);
                                if(this.flxx_fsss_add[i]['zbflId'] == this.now_data['data']['zbflId'] ){
                            ...
                            //如果 当前分类信息不为空，则将当前分类信息添加到flxx_fsss_add中

                            if(this.flxx_fsss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] == undefined){
                                this.flxx_fsss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                            }
                    ...
                    if((this.now_data['data']['bz'] != null && this.now_data['data']['bz'] != "") || (this.now_data['data']['sl'] != null && this.now_data['data']['sl'] != "" ) || (this.now_data['data']['sl'] != null && this.now_data['data']['sl'] != "") || (this.now_data['data']['fs'] != null && this.now_data['data']['fs'] != ""  )  ){
                        ...
                        this.name_active_data['flbmxList'][this.name_active_data['id']+now_data['data']['zbflId']] = {'id':now_data['data']['id'],'sl':now_data['data']['sl'],'bz':now_data['data']['bz'],'fssslbdm':now_data['data']['fssslbdm'],'zbflId':now_data['data']['zbflId'],'ssfsssjbxxId':this.name_active_data.id};
                ...
                this.name_active_data['flbmxList'][this.now_data['data']['zbflId']] = {'id':this.now_data['data']['id'],'sl':this.now_data['data']['sl'],'bz':this.now_data['data']['bz'],'fssslbdm':this.now_data['data']['fssslbdm'],'zbflId':this.now_data['data']['zbflId'],'ssfsssjbxxId':this.name_active_data.id};
            ...
            for(let i in this.fsss_name_active_base_copy[0]){

                this.fsss_name_active_base_copy[0][i] = this.InputChange.showCheck4(this.fsss_name_active_base_copy[0][i],res,'fssslbdm');
            }
    ...
    TreeTableClick(e,data,now_data,i){
        ...
        this.fssslbdm = e.node.data.fssslbdm;
        console.log(this.fssslbdm);
    ...
    delGgmx(){
        ...
        if(this.ggmx && this.ggmx.length >0){
            ...
            console.log(this.ggmx);
            console.log(this.flxx_fsss_add);
            // console.log(this.list_fsss_ggmx_del);
            // console.log(this.list_fsss_ggmx_edit);
            // console.log(this.list_fsss_ggmx_add);
    ...
    selectedTypea3(): void {
        ...
        name_active_base_copy.forEach((value,index,arr)=>{
            ...
            value = this.DataProcessing.returnTreeGgmx(value,'fssslbdm');
            value.forEach((v,i,a)=>{
    ...
    searchList(searchKeyword){
        ...
        if(this.fsssList != undefined && this.fsssList.length>0){
            this.name_active_base.forEach((value,index,arr)=>{
                ...
                this.InputChange.delSearchRsut(this.fsss_name_active_base_copy[0][index],'searchRsut');
...
//==================下面下拉列表 ================================
    ...
    eventQsr(e){
        ...
        this.selectName(this.name_active_data,this.name_active_key);
        this.fsssList[this.name_active_key] = this.name_active_data;
        ...
        if(this.name_active_data.qsrId.toString().length == 32){
            ...
            if(this.init_fsss_data['datalist'][this.name_active_key] != undefined && this.init_fsss_data['datalist'][this.name_active_key]['id']==this.name_active_data.id ){

                delete this.update_fsss_data[this.name_active_key];

                this.del_fsss_data[this.name_active_key] = ({'id':this.init_fsss_data['datalist'][this.name_active_key]['id']});
                ...
                for(let i in this.flxx_fsss_add){
                    console.log(this.flxx_fsss_add[i]['ssfsssjbxxId']);
                    console.log(this.name_active_data['id']);
                    if(this.flxx_fsss_add[i]['ssfsssjbxxId'] ==  this.name_active_data['id']){
                        delete this.flxx_fsss_add[i];
                    }
                }
                ...
                for(let i in this.add_ggmx_data){
                    console.log(this.add_ggmx_data[i]['ssfsssjbxxId']);
                    console.log(this.name_active_data['id']);
                    if(this.add_ggmx_data[i]['ssfsssjbxxId'] ==  this.name_active_data['id']){
                        delete this.add_ggmx_data[i];
                    }
                }
            ...
            if(this.listHcyAdd[this.name_active_data.qsrId]['listFsssAdd'][this.name_active_data.id]){
                delete this.listHcyAdd[this.name_active_data.qsrId]['listFsssAdd'][this.name_active_data.id];
        ...
        if(e.toString().length == 32){
            ...
            let res =  this.InputChange.get_select_change(this.name_active_data,this.name_active_key,this.init_fsss_data,this.update_fsss_data,this.add_fsss_data);
            ...
            //this.del_fsss_data.push({'id':temp_qsrid});
            console.log(this.add_fsss_data);
            ...
            for(let i in res2){
                ...
                if(this.add_fsss_data[this.name_active_data['id']]['flbmxList'] == null){
                    this.add_fsss_data[this.name_active_data['id']]['flbmxList'] = new Array();
                }



                if(this.add_fsss_data[this.name_active_data['id']]['flbmxList'][i]){
                    this.add_fsss_data[this.name_active_data['id']]['flbmxList'][i] = new Array();
                }

                this.add_fsss_data[this.name_active_data['id']]['flbmxList'][i] = res2[i];

            }

            this.name_active_data = this.add_fsss_data[this.name_active_data['id']];
    ...
    showZydlBlock() {
        ...
        if(this.type != 'view'){
            ...
            if (!this.zydlTableList) {
                ...
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_FSSSJBXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        this.zydlTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_FSSSJBXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
    ...
    showDcfwBlock() {
        ...
        if(this.type != 'view'){
            ...
            if (!this.dcfwTableList) {
                ...
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_FSSSJBXX&column=DCFWDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        this.dcfwTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_FSSSJBXX&column=DCFWDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        console.log(res['returnObject']);
...
//行政区划代码
    ...
    getChildytjjg(event) {
        ...
        this.ggmx[this.GgmxIndex]['fssslbdm'] = event.node.fssslbdm;
    ...
    ToggleFlxxAdd(arr,e,ssfsssjbxxId){
        for (let item of arr) {
            console.log(item);
            if (item.children) {

                this.ToggleFlxxAdd(item.children,e,ssfsssjbxxId);
            } else {

                if((item['data']['sl'] != null && item['data']['sl'] != "" )){

                    this.toggle_flxx_add[ssfsssjbxxId+item.data['zbflId']] = new Array();
                    this.toggle_flxx_add[ssfsssjbxxId+item.data['zbflId']] = {'sl':item['data']['sl'],'bz':item['data']['bz'],'fssslbdm':item['data']['fssslbdm'],'zbflId':item['data']['zbflId'],'qsrId':e,'ssfsssjbxxId':ssfsssjbxxId};
    ...
    ToggleFlxxAdd2(arr,e,ssfsssjbxxId){
        this.toggle_flxx_add = new Array();

        arr.forEach((value,index,arr)=>{
            this.ToggleFlxxAdd(arr[index],e,ssfsssjbxxId);
