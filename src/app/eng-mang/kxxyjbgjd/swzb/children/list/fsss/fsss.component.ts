...
export class FsssComponent implements OnInit {
    ...
    public update_fsss_data =new Array(); //对比后修改的数据
    ...
    showTable: any = 1;
    constructor(public InputChange:InputChangeService,public HttpService:HttpService,public selectList:SelectListHttpService,public ValuChangeService:ValuChangeService,public DataProcessing: DataProcessingService) {
        this.types = [];
    ...
    ngOnInit() {
        ...
        //  专业大类 下拉列表
        this.selectList.getSelectList('B_FSSSJBXX', 'ZYDLDM', this.ssgcdm, this.ssxzqhdm).then(data => {
            this.zydl_list = data;
        });
        //  房屋性质 下拉列表
    ...
    getChildEvent1(event){
        ...
        this.name_active_data['list'] = JSON.parse(JSON.stringify(this.init_fsss_data['baseList']));
        ...
        if((typeof this.fsssList) != 'undefined' && this.fsssList.length>0){
            this.fsssList.push(new TreesData('','','','','','','','','',event,'','','','','',0,'','','','','','','','',name,'','',''));
    ...
    deleteSlect(){
        ...
        // if(this.fsssList[0] != undefined && this.name_active_key>0){
        console.log(this.name_active_data);
    ...
    dcfw_show(){
        ...
        if(this.dcfw_list_is_show == 0){
            ...
            this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_FSSSJBXX&column=DCFWDM&gcdm=${this.qshflId.ssgcdm}&xzqhdm=${this.qshflId.ssxzqhdm}`).then((data)=>{
