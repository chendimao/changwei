...
export class FyssComponent implements OnInit {
    ...
    public del_fyss_data = new Array(); //删除的数据
    public init_fyss_data; //初始化的数据
    public update_fyss_data; //对比后修改的数据
    public add_fyss_data =new Array(); //新增的数据
    ...
    public bj_fyss_data; //初始化数据标记
    public bj_fyss_new_data; //初始化新增数据标记
    public flxx_fyss_update = new Array(); //分类信息
    public flxx_fyss_add = new Array(); //分类信息新增
    public flxx_fyss_del = new Array(); //分类信息删除
    ...
    public fyss_name_active_base_copy;
    public szxzqugldm;
...
// 订阅表单值改变事件
            this.person2.valueChanges.subscribe(data => {
                    ...
                    if(this.fyssList.length>0){
                        if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
                            ...
                            let res =  this.InputChange.get_value_change(this.name_active_data,this.name_active_key,this.init_fyss_data,this.update_fyss_data,this.add_fyss_data);
    ...
    ngOnInit() {
        ...
        //
        // let oninit_res = this.TabCommon.Oninit(this.data,this.name_active_data,this.fyssList,this.name_active_base,this.bj_fyss_data);
        //
        ...
        // this.fyssList = oninit_res.List;
        // this.bj_fyss_data = oninit_res.bj_data;

        if(this.data['returnObject']['datalist']){
            if(this.data['returnObject']['datalist'][0] !=undefined ){


                this.name_active_data = [ new fyssData('','','','','','',new Date().getTime(),'','','','',[],'','','','','','','','','','','','','','','','','','','','','','') ];

                this.fyssList = this.data['returnObject']['datalist'];  //户成员信息
                console.log(this.fyssList);
                this.name_active_data = this.fyssList[0];   //默认选中第一个户成员
                ...
                if(this.init_fyss_data['datalist'] != undefined && this.init_fyss_data['datalist'].length>0){

                    this.init_name_active_base = this.init_fyss_data['datalist'][0]['list'];
                }
...
// =============== end 规格明细  =======================
        ...
        if(this.bj_fyss_data == 1) {
            ...
            if(this.init_fyss_data['datalist'] != undefined && this.init_fyss_data['datalist'].length >0){
                this.init_fyss_data['datalist'].forEach((value,index,arr)=>{
                    value.list.forEach((val,i,ar)=>{
                        ar[i] = this.DataProcessing.returnTreeTable(this.DataProcessing.replaceChildlValue(val, 'childList', 'children', '', ''));
            ...
            this.init_fyss_data['baseList'].forEach((val, i, ar) => {
                ar[i] = this.DataProcessing.returnTreeTable(this.DataProcessing.replaceChildlValue(val, 'childList', 'children', '', ''));
        ...
        if(this.name_active_base != null && this.name_active_base != "" && this.name_active_base.length>0){
            ...
            if(this.fyss_name_active_base_copy[0] == 0){

                this.fyss_name_active_base_copy[0] = _.cloneDeep(this.name_active_base);
                //this.fyss_name_active_base_copy2 = _.cloneDeep(this.name_active_base);
            ...
            console.log(this.name_active_base);
            console.log(this.fyss_name_active_base_copy);
        ...
        console.log(this.update_fyss_data);
        console.log(this.add_fyss_data);
        console.log(this.del_fyss_data);
...
//选择name改变样式
    selectName(name,i){
        ...
        if(this.fyssList != undefined && this.fyssList.length>0){
            console.log(this.fyssList.length);
            console.log(this.fyssList);
            ...
            if(this.init_fyss_data['datalist']!= undefined && this.init_fyss_data['datalist'][this.name_active_key] != undefined && this.init_fyss_data['datalist'][this.name_active_key]['list']){
                this.init_name_active_base = _.cloneDeep(this.init_fyss_data['datalist'][this.name_active_key]['list']);
            }else{
    ...
    addfyss(event){
        ...
        this.name_active_base = JSON.parse(JSON.stringify(this.init_fyss_data['baseList']));
        console.log(this.name_active_base);
        //转换数据格式
        ...
        if( this.fyssList != undefined && this.fyssList.length>0){
            this.fyssList.push(new fyssData('','','','','','',new Date().getTime(),'','',event,'',[],'','','','','','','','','','','','',name,'','','','','','','','',''));
            this.name_active_data = this.fyssList[this.fyssList.length-1];
            this.name_active_data['list'] = this.name_active_base;

            this.name_active_key = this.fyssList.length-1;

        }else{
            this.fyssList.push(new fyssData('','','','','','',new Date().getTime(),'','',event,'',[],'','','','','','','','','','','','',name,'','','','','','','','',''));
            this.name_active_data = this.fyssList[0];
            this.name_active_data['list'] = this.name_active_base;
        ...
        if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
            ...
            for(let i in this.add_fyss_data){
                if(i == this.name_active_data['id']){
                    this.add_fyss_data[i] = this.name_active_data;
                }
            }
        ...
        console.log(this.flxx_fyss_del);
        console.log(this.flxx_fyss_add);
        console.log(this.flxx_fyss_update);
    ...
    deleteSlect(){
        ...
        if(this.fyssList.length>0){
            ...
            if(this.name_active_data.qsrId.toString().length != 32) { // 如果权属人ID等于32位也就是说 在数据库中已经存在的户成员
                ...
                if(this.add_fyss_data[this.name_active_data['id']]){
                    delete this.add_fyss_data[this.name_active_data['id']];
                }

                this.del_fyss_data.push({'id':this.name_active_data['id']});

                //如果删除的户成员在修改列表中存在，则从修改列表中删除

                this.fyssList.forEach((value, index, arr) => {
                    if (value['id'] == this.name_active_data['id']) {
                        delete this.update_fyss_data[index];
                    }
                });
                console.log(this.del_fyss_data);
            ...
            //
            // this.fyssList.splice(this.name_active_key,1);
            // if(this.fyssList[0] != undefined && this.fyssList.length>0  ){
            //
            //     if(this.name_active_key == 0){
            //         this.name_active_data = this.fyssList[this.name_active_key+1];
            ...
            //
            //
            // }else if(this.fyssList[0] != undefined && this.name_active_key==0){
            //
            //     this.name_active_data = this.fyssList[this.name_active_key+1];
            //     this.name_active_key = this.name_active_key+1;
            //
            ...
            if(this.fyssList[0] != undefined){
                if(this.name_active_key == 0){
                    this.name_active_data = this.fyssList[0];
                }else if(this.name_active_key == this.fyssList.length){

                    this.name_active_key = this.name_active_key-1;
                    this.name_active_data = this.fyssList[this.name_active_key];
    ...
    Tree_update(data,now_data,i){
        this.fysslbdm = now_data['data']['fysslbdm'];
        if(this.fyssList != undefined && this.fyssList.length>0){
            ...
            this.ssfyssjbxxId = this.name_active_data['id'];
                ...
                if(this.name_active_data.id != undefined && this.name_active_data.id.toString().length == 32){
                    ...
                    if(this.now_data['data']['id'] != null && this.now_data['data']['id'] != ""){
                        ...
                        if(this.now_data['data']['sl'] == null || this.now_data['data']['sl'] == ""){
                            ...
                            //删除当前分类信息，将删除id放入flxx_fyss_del字段
                            this.flxx_fyss_del[this.now_data['data']['id']] ={id:this.now_data['data']['id']};


                            //如果之前该分类信息有更新，则删除flxx_fyss_update中该分类信息的字段
                            for(let i in this.flxx_fyss_update){

                                console.log(this.flxx_fyss_update[i].id);
                                console.log(this.now_data['data']['id']);
                                if(this.flxx_fyss_update[i].id == this.now_data['data']['id']){
                                    delete this.flxx_fyss_update[i];
                                    console.log(this.flxx_fyss_update[i]);
                                }

                            }


                            for(let i  in this.flxx_fyss_add){
                                console.log(this.flxx_fyss_add[i]['zbflId']);
                                console.log(this.now_data['data']['zbflId']);
                                if(this.flxx_fyss_add[i]['zbflId'] == this.now_data['data']['zbflId'] ){
                                    delete this.flxx_fyss_add[i];
                                }
                            }
                            ...
                            for(let i in this.flxx_fyss_del){

                                if(this.flxx_fyss_del[i]['id'] == this.now_data['data']['id']){
                                    is_del = 1;
                                }
                            }


                            //如果不在 flxx_fyss_del 中，则更新flxx_fyss_update
                            if(is_del ==0){

                                let res = this.InputChange.IsWith(this.init_name_active_base[i],this.fysslbdm,this.ssfyssjbxxId,now_data['data']['sl'],'sl','fysslbdm','ssfyssjbxxId');
                                let res2 = this.InputChange.isWithBz2(this.init_name_active_base[i],now_data,now_data['data']['bz'],'bz');

                                if(res == false || res2 == false){
                                    this.flxx_fyss_update[this.now_data['data']['id']] = new Array();
                                    this.flxx_fyss_update[this.now_data['data']['id']] = {'id':this.now_data['data']['id'],'sl':this.now_data['data']['sl'],'bz':this.now_data['data']['bz'],'fysslbdm':this.now_data['data']['fysslbdm'],'zbflId':this.now_data['data']['zbflId'],'ssfyssjbxxId':this.name_active_data.id};

                                }else{

                                    delete this.flxx_fyss_update[this.now_data['data']['id']];
                                ...
                                if(this.flxx_fyss_add[this.name_active_data['id']+this.now_data['data']['zbflId']]){
                                    this.flxx_fyss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                                }


                                this.flxx_fyss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                                this.flxx_fyss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = {'sl':this.now_data['data']['sl'],'bz':this.now_data['data']['bz'],'fysslbdm':this.now_data['data']['fysslbdm'],'zbflId':this.now_data['data']['zbflId'],'ssfyssjbxxId':this.name_active_data.id};
                        ...
                        if((this.now_data['data']['sl'] == null || this.now_data['data']['sl'] == "") &&  (this.now_data['data']['bz'] == null || this.now_data['data']['bz'] == "")){
                            ...
                            //如果 当前分类信息为空，则判断flxx_fyss_add中有没有该分类信息，如果有则删除
                            for(let i  in this.flxx_fyss_add){
                                console.log(this.flxx_fyss_add[i]['zbflId']);
                                console.log(this.now_data['data']['zbflId']);
                                if(this.flxx_fyss_add[i]['zbflId'] == this.now_data['data']['zbflId'] ){
                            ...
                            //如果 当前分类信息不为空，则将当前分类信息添加到flxx_fyss_add中

                            if(this.flxx_fyss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] == undefined){
                                this.flxx_fyss_add[this.name_active_data['id']+this.now_data['data']['zbflId']] = new Array();
                            }
                    ...
                    if((this.now_data['data']['bz'] != null && this.now_data['data']['bz'] != "") || (this.now_data['data']['sl'] != null && this.now_data['data']['sl'] != "" ) || (this.now_data['data']['sl'] != null && this.now_data['data']['sl'] != "") || (this.now_data['data']['fs'] != null && this.now_data['data']['fs'] != ""  )  ){
                        ...
                        this.name_active_data['flbmxList'][this.name_active_data['id']+now_data['data']['zbflId']] = {'id':now_data['data']['id'],'sl':now_data['data']['sl'],'bz':now_data['data']['bz'],'fysslbdm':now_data['data']['fysslbdm'],'zbflId':now_data['data']['zbflId'],'ssfyssjbxxId':this.name_active_data.id};
            ...
            console.log(this.add_fyss_data);
            console.log(this.ssfyssjbxxId);
            console.log(this.name_active_base);

            console.log(this.isShow);
            let res = this.InputChange.showCheck3(this.name_active_base,'fysslbdm');
            console.log(res);

            for(let i in this.fyss_name_active_base_copy[0]){

                this.fyss_name_active_base_copy[0][i] = this.InputChange.showCheck4(this.fyss_name_active_base_copy[0][i],res,'fysslbdm');
            }
    ...
    TreeTableClick(e,data,now_data,i){
        ...
        this.fysslbdm = e.node.data.fysslbdm;
        console.log(this.fysslbdm);
    ...
    DatatableClick(e){
        ...
        this.ggmx[this.GgmxIndex]['zdxmc'] = this.searchService.searchByRegExp(this.ggmx[this.GgmxIndex]['fysslbdm'],this.tableList,'fysslbdm')[0]['zdxmc'];
    ...
    addGgmx(e){
        ...
        console.log(this.ggmx);
        this.ggmx.push(new Ggmx(this.ggmx.length==0?0:this.ggmx[this.ggmx.length - 1]['xh']+1,'','','','','','','','','','','','','','','','','','','','','', this.tableList[0]['fysslbdm'],this.ssfyssjbxxId));
    ...
    delGgmx(){
        ...
        if(this.ggmx && this.ggmx.length >0){
            ...
            console.log(this.ggmx);
            console.log(this.flxx_fyss_add);
            // console.log(this.list_fyss_ggmx_del);
            // console.log(this.list_fyss_ggmx_edit);
            // console.log(this.list_fyss_ggmx_add);
    ...
    selectedTypea3(): void {
        ...
        name_active_base_copy.forEach((value,index,arr)=>{
            ...
            value = this.DataProcessing.returnTreeGgmx(value,'fysslbdm');
            value.forEach((v,i,a)=>{
    ...
    searchList(searchKeyword){
        ...
        if(this.fyssList != undefined && this.fyssList.length>0){
            this.name_active_base.forEach((value,index,arr)=>{
                ...
                this.InputChange.delSearchRsut(this.fyss_name_active_base_copy[0][index],'searchRsut');
...
//==================下面下拉列表 ================================
    ...
    eventQsr(e){
        ...
        this.selectName(this.name_active_data,this.name_active_key);
        this.fyssList[this.name_active_key] = this.name_active_data;
        ...
        if(this.name_active_data.qsrId.toString().length == 32){
            ...
            if(this.init_fyss_data['datalist'][this.name_active_key] != undefined && this.init_fyss_data['datalist'][this.name_active_key]['id']==this.name_active_data.id ){

                delete this.update_fyss_data[this.name_active_key];

                this.del_fyss_data[this.name_active_key] = ({'id':this.init_fyss_data['datalist'][this.name_active_key]['id']});
                ...
                for(let i in this.flxx_fyss_add){
                    console.log(this.flxx_fyss_add[i]['ssfyssjbxxId']);
                    console.log(this.name_active_data['id']);
                    if(this.flxx_fyss_add[i]['ssfyssjbxxId'] ==  this.name_active_data['id']){
                        delete this.flxx_fyss_add[i];
                    }
                }
                ...
                for(let i in this.add_ggmx_data){
                    console.log(this.add_ggmx_data[i]['ssfyssjbxxId']);
                    console.log(this.name_active_data['id']);
                    if(this.add_ggmx_data[i]['ssfyssjbxxId'] ==  this.name_active_data['id']){
                        delete this.add_ggmx_data[i];
                    }
                }
        ...
        if(e.toString().length == 32){
            ...
            //this.del_fyss_data.push({'id':temp_qsrid});
            console.log(this.add_fyss_data);
            ...
            for(let i in res2){
                ...
                if(this.add_fyss_data[this.name_active_data['id']]['flbmxList'] == null){
                    this.add_fyss_data[this.name_active_data['id']]['flbmxList'] = new Array();
                }



                if(this.add_fyss_data[this.name_active_data['id']]['flbmxList'][i]){
                    this.add_fyss_data[this.name_active_data['id']]['flbmxList'][i] = new Array();
                }

                this.add_fyss_data[this.name_active_data['id']]['flbmxList'][i] = res2[i];

            }

            this.name_active_data = this.add_fyss_data[this.name_active_data['id']];
    ...
    showZydlBlock() {
        ...
        if(this.type != 'view'){
            ...
            if (!this.zydlTableList) {
                ...
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_FYSSJBXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        this.zydlTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_FYSSJBXX&column=ZYDLDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
    ...
    showDcfwBlock() {
        ...
        if(this.type != 'view'){
            ...
            if (!this.dcfwTableList) {
                ...
                this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_FYSSJBXX&column=DCFWDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        this.dcfwTreeList = this.DataProcessing.replaceChildlValue(res['returnObject'], 'listZdk', 'children', 'mc', 'label');
                    });
                this.HttpService.get(`zdk/getZdkByTableAndColumn2?tableName=B_FYSSJBXX&column=DCFWDM&gcdm=${this.ssgcdm}&xzqhdm=${this.ssxzqhdm}`)
                    .then((res) => {
                        console.log(res['returnObject']);
...
//行政区划代码
    ...
    getChildytjjg(event) {
        ...
        this.ggmx[this.GgmxIndex]['fysslbdm'] = event.node.fysslbdm;
    ...
    ToggleFlxxAdd(arr,e,ssfyssjbxxId){
        for (let item of arr) {
            console.log(item);
            if (item.children) {

                this.ToggleFlxxAdd(item.children,e,ssfyssjbxxId);
            } else {

                if((item['data']['sl'] != null && item['data']['sl'] != "" )){

                    this.toggle_flxx_add[ssfyssjbxxId+item.data['zbflId']] = new Array();
                    this.toggle_flxx_add[ssfyssjbxxId+item.data['zbflId']] = {'sl':item['data']['sl'],'bz':item['data']['bz'],'fysslbdm':item['data']['fysslbdm'],'zbflId':item['data']['zbflId'],'qsrId':e,'ssfyssjbxxId':ssfyssjbxxId};
    ...
    ToggleFlxxAdd2(arr,e,ssfyssjbxxId){
        this.toggle_flxx_add = new Array();

        arr.forEach((value,index,arr)=>{
            this.ToggleFlxxAdd(arr[index],e,ssfyssjbxxId);
