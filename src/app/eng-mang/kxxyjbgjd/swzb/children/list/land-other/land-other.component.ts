...
export class LandOtherComponent implements OnInit {
    ...
    public update_land_other_data =new Array(); //对比后修改的数据
    ...
    ngOnInit() {
        ...
        //  专业大类 下拉列表
        this.selectList.getSelectList('B_TDFZWJBXX', 'ZYDLDM', this.ssgcdm, this.ssxzqhdm).then(data => {
            this.zydl_list = data;
        });
        //  房屋性质 下拉列表
    ...
    getChildEvent1(event){
        this.name_active_data['list'] = JSON.parse(JSON.stringify(this.init_land_other_data['baseList']));
        ...
        if((typeof this.land_otherList) != 'undefined' && this.land_otherList.length>0){
            this.land_otherList.push(new TreesData('','','','','','','','','',event,'','','','','',0,'','','','','','','','',name,'','',''));
    ...
    deleteSlect(){
        ...
        // if(this.land_otherList[0] != undefined && this.name_active_key>0){
        console.log(this.name_active_data);
    ...
    dcfw_show(){
        ...
        if(this.dcfw_list_is_show == 0){
            ...
            this.HttpService.get(`zdk/getZdkByTableAndColumn?tableName=B_TDFZWJBXX&column=DCFWDM&gcdm=${this.qshflId.ssgcdm}&xzqhdm=${this.qshflId.ssxzqhdm}`).then((data)=>{
